...
def test_case_c_one_file_many_chapters_is_still_one_output(tmp_path) -> None:
    ...
    assert "Chapter 1 One" in text
    assert "Chapter 2 Two" in text


@pytest.mark.xfail(reason="Phase 3 case C: split one XHTML into many chapter files", strict=False)
...
def test_case_b_many_files_one_nav_entry_stay_separate(tmp_path) -> None:
    epub = write_bytes(tmp_path / "split.epub", make_split_chapter_files())
    book_dir, _report = convert_epub(epub, tmp_path / "out-split")
    assert len(list((book_dir / "content").glob("*.md"))) == 2


@pytest.mark.xfail(reason="Phase 3 case B: merge split XHTML into one chapter file", strict=False)
...
def test_virtual_part_has_no_section_file_in_phase1(tmp_path) -> None:
...
@pytest.mark.xfail(reason="Phase 3: emit a virtual part node with children, no content file", strict=False)
